...
let batchNum = 0, downloadNum = 0, allInvoices = [];
 ...
 */
async function downloadInvoices(directory, unixStart, unixEnd, limit = 5, starting_after) {
	...
	if (has_more) {
		await downloadInvoices(directory, unixStart, unixEnd, limit, data[data.length-1].id)
	}
}
...
async function main () {
	...
	console.log(`Downloading invoices between ${startDate.toFormat("yyyy-MM-dd")} - ${endDate.toFormat("yyyy-MM-dd")}`)
	...
	await downloadInvoices(dDir, Math.round(startDate.toSeconds()), Math.round(endDate.toSeconds()))
	console.log(`Done. Ran ${batchNum} batches and downloaded ${downloadNum} PDFs.`)

	// Calculate & display totals
	const totals = allInvoices
		.reduce(([subtotal, tax, total], invoice) => {
			return [
